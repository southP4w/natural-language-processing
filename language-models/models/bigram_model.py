...
class BigramModel:
    ...
    def compute_probability_raw(self, word_0: str, word_1: str):
        return (self.V_1.get((word_0, word_1), 0)) / (self.V_0.get(word_0, 1))

    def compute_probability_log(self, sentence: iter):
        ...
        for i in range(len(s) - 1):
            ...
            try:
                log_2_p += math.log2(p)
                print(f"Bigram: ({w_0}| {w_1}), Raw Probability: {p * 100:.4f}%, log_2 Probability: {math.log2(p):.4f}%")
            except ValueError:
                print(f"BIGRAM: ({w_0}| {w_1}), RAW PROBABILITY: {p} ZERO")

        return log_2_p

    def compute_probability_smoothed(self, sentence: iter):
        s = ["<s>"] + sentence.strip().lower().split() + ["</s>"]
        log_2_p = 0
        for i in range(len(s) - 1):
            w_0, w_1 = s[i], s[i + 1]
            ...
            try:
                log_2_p += math.log2(p)
                print(f"Bigram: ({w_0}| {w_1}), Smoothed Probability: {p * 100:.4f}%, Smoothed log_2 Probability: {math.log2(p):.4f}%")
            except ValueError:
                print(f"BIGRAM: ({w_0}| {w_1}), SMOOTHED PROBABILITY: {p}% ZERO")

        return log_2_p

    def compute_perplexity_of_sentence(self, sentence: iter):
        s = ["<s>"] + sentence.strip().lower().split() + ["</s>"]
        log_2_p = 0
        for i in range(len(s) - 1):
            w_0, w_1 = s[i], s[i + 1]
        ...
        PP = 2**(-log_2_p / len(s) - 1)

        return PP

    def compute_perplexity_smoothed_of_sentence(self, sentence: iter):
        s = ["<s>"] + sentence.strip().lower().split() + ["</s>"]
        log_2_p = 0
        for i in range(len(s) - 1):
            w_0, w_1 = s[i], s[i + 1]
    ...
    def smooth_probabilities(self, word_0, word_1):
